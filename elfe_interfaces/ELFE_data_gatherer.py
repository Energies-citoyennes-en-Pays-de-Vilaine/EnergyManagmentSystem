...
def get_heater_consumer(timestamp : int, calculationParams: CalculationParams) -> List[HeaterConsumer]:
	...
	for heater in elfe_heater:
		...
		T_ext_response = fetch(db_credentials["EMS"], ("SELECT wheather_timestamp, temperature FROM initialweather WHERE wheather_timestamp >= %s",[get_round_timestamp()]))
		T_ext = np.array([T_ext_response[i],T_ext_response[i]] + [T_ext_response[i][0] for i in range(len(calculationParams.get_time_array()))])
		print(T_ext)
